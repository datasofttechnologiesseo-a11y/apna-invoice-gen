        ...
        <div class="max-h-96 overflow-y-auto">
            @if ($totalCount === 0)
                <div class="px-4 py-10 text-center">
                    <div class="w-12 h-12 mx-auto rounded-full bg-money-50 ring-1 ring-money-100 flex items-center justify-center text-money-600 mb-3">
            ...
            @if ($overdue->isNotEmpty())
                <div class="px-4 pt-3 pb-1">
                    <div class="text-[10px] font-bold uppercase tracking-widest text-danger-600 flex items-center gap-1.5">
                ...
                <ul class="divide-y divide-gray-50">
                    @foreach ($overdue as $inv)
                        <a href="{{ route('invoices.show', $inv) }}" class="block px-4 py-2.5 hover:bg-danger-50/40 transition">
                            <div class="flex items-baseline justify-between gap-2">
                                <div class="font-mono text-xs font-semibold text-gray-900 truncate">{{ $inv->invoice_number ?? '#' . $inv->id }}</div>
                                <div class="font-mono text-xs font-bold text-danger-700 tabular-nums whitespace-nowrap">₹{{ inr($inv->balance) }}</div>
                            </div>
                            <div class="flex items-baseline justify-between gap-2 mt-0.5">
                                ...
                                <div class="text-[10px] text-danger-600 font-semibold whitespace-nowrap">{{ $inv->due_date?->diffForHumans() }}</div>
                            </div>
                        </a>
                    @endforeach
                </ul>
            ...
            @if ($recentPayments->isNotEmpty())
                <div class="px-4 pt-3 pb-1">
                    <div class="text-[10px] font-bold uppercase tracking-widest text-money-700 flex items-center gap-1.5">
                ...
                <ul class="divide-y divide-gray-50">
                    @foreach ($recentPayments as $p)
                        <a href="{{ $p->invoice ? route('invoices.show', $p->invoice) : '#' }}" class="block px-4 py-2.5 hover:bg-money-50/40 transition">
                            <div class="flex items-baseline justify-between gap-2">
