...
<div class="bg-white rounded-xl shadow-card ring-1 ring-brand-100 overflow-hidden">
    {{-- Header strip — gradient gives the card weight without screaming.
         Saffron accent on the icon for Indian-flag warmth without being kitsch. --}}
    <div class="px-5 py-3 bg-gradient-to-r from-brand-50 to-saffron-50 border-b border-brand-100 flex items-center justify-between gap-3">
        <div class="flex items-center gap-2">
            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-saffron-100 text-saffron-700">
                @if ($isPaid)
    ...
    <div class="p-4 grid grid-cols-1 sm:grid-cols-2 @if (! $isPaid && ! $isDraft) md:grid-cols-4 @else md:grid-cols-3 @endif gap-3">
        ...
        @elseif ($isPaid)
            ...
            <a href="{{ route('invoices.create') }}" class="group flex items-center gap-3 px-4 py-3 bg-saffron-600 hover:bg-saffron-700 text-white rounded-lg shadow-sm text-left transition">
                <span class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-white/15 group-hover:bg-white/25 transition">
        ...
        @else
            ...
            @if ($waLink)
                <a href="{{ $waLink }}" target="_blank" rel="noopener" class="group flex items-center gap-3 px-4 py-3 bg-[#25D366] hover:bg-[#1ebe5b] text-white rounded-lg shadow-sm text-left transition">
            ...
            <a href="#record-payment" class="group flex items-center gap-3 px-4 py-3 bg-white ring-1 ring-money-200 hover:ring-money-400 hover:bg-money-50 rounded-lg text-left transition">
                ...
                <span class="flex-1">
                    <span class="block font-semibold text-gray-900 leading-tight">Record payment</span>
            ...
            <a href="{{ route('invoices.create') }}" class="group flex items-center gap-3 px-4 py-3 bg-white ring-1 ring-gray-200 hover:ring-saffron-300 hover:bg-saffron-50 rounded-lg text-left transition">
                <span class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-saffron-100 text-saffron-700 group-hover:bg-saffron-600 group-hover:text-white transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                </span>
                <span class="flex-1">
