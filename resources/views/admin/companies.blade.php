<x-layouts.admin title="Companies" subtitle="{{ $companies->total() }} total · every business entity on the platform">
    <x-slot:action>
        <form method="GET" class="flex gap-2">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search name or GSTIN" class="w-56 border-gray-300 rounded-md shadow-sm text-sm focus:border-brand-600 focus:ring-brand-600" aria-label="Search name or GSTIN">
            @if (request('search'))
