                ...
                <div class="hidden lg:ms-8 lg:flex lg:items-center lg:gap-1">
                    @foreach ($navItems as $item)
                        <a href="{{ $item['href'] }}" @class([
                            'group inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap',
                            'bg-brand-50 text-brand-700' => $item['active'],
                            'text-gray-600 hover:bg-gray-100 hover:text-gray-900' => ! $item['active'],
                        ])>
                            <svg class="w-4 h-4 {{ $item['active'] ? 'text-brand-600' : 'text-gray-400 group-hover:text-gray-500' }} transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
