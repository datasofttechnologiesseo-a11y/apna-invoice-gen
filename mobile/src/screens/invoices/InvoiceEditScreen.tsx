...
type Props = NativeStackScreenProps<InvoicesStackParamList, 'InvoiceEdit'>;
...
export default function InvoiceEditScreen({ route, navigation }: Props) {
  ...
  // When editing, hydrate from the existing invoice.
  ...
  // Live total preview (mirrors InvoiceCalculator: per-line tax, summed).
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return isEdit ? updateInvoice(editId!, payload) : createInvoice(payload);
    },
    onSuccess: (inv) => {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: 16 }}>
      ...
      <Card>
        ...
        <Text style={styles.meta}>Date: {invoiceDate}</Text>
      </Card>
      ...
      {items.map((it, idx) => (
        <Card key={it._key} style={{ marginBottom: 12 }}>
          ...
          <Pressable style={styles.linkPick} onPress={() => setProductModalFor(it._key)}>
      ...
      <Card style={{ marginTop: 16 }}>
        ...
        <Text style={styles.previewNote}>Final totals are computed on the server.</Text>
      </Card>
...
const styles = StyleSheet.create({
  ...
  previewNote: { fontSize: 11, color: colors.muted, marginTop: 8, fontStyle: 'italic' },
